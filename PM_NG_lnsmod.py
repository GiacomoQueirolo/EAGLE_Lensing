...
Gal = prep_Gal(Gal)
dens_Ms_kpc2,radius,dP,dxdy,z_source,cosmo  = get_dens_map_rotate_hist(Gal,plot=False)
...
Xstar,Ystar,_ =  np.transpose(Gal.stars["coords"])# Mpc
#RAstar,DECstar = Xstar.to("kpc").value*arcXkpc,Ystar.to("kpc").value*arcXkpc
Xgas,Ygas,_   =  np.transpose(Gal.gas["coords"]) # Mpc
#RAgas,DECgas = Xgas.to("kpc").value*arcXkpc,Ygas.to("kpc").value*arcXkpc
Xdm,Ydm,_     =  np.transpose(Gal.dm["coords"]) # Mpc
#RAdm,DECdm = Xdm.to("kpc").value*arcXkpc,Ydm.to("kpc").value*arcXkpc
Xbh,Ybh,_     =  np.transpose(Gal.bh["coords"]) # Mpc
#RAbh,DECbh = Xbh.to("kpc").value*arcXkpc,Ybh.to("kpc").value*arcXkpc


RAs  = np.concatenate([Xstar,Xgas,Xdm,Xbh])*u.Mpc.to("kpc")*arcXkpc.to('"/kpc')
DECs = np.concatenate([Ystar,Ygas,Ydm,Ybh])*u.Mpc.to("kpc")*arcXkpc.to('"/kpc')
RAs  = RAs.value
DECs = DECs.value
...
ax[0].set_title("Lensed image (lnstr)")
plt.savefig(lens_dir+"/lensed_im_PM.pdf")
plt.show()
