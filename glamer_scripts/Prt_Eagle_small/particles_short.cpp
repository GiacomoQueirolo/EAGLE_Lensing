 /*
   short version -> only consider the first 100 particles
...
int main(int arg,char **argv){
  ...
  std::string particle_file = "data/particles_EAGLE_short.csv" ; //only consider the first 100 particles  // csv4 -> x,y,z,M # could be csv6 w smoothin and particle type but DM has no smoothing scale
  ...
  Point_3d<double> center;
  {
    ...
    */
    ...
    //-> will be 0,0 by construction bc it recenter the coordinate to that
    center[0] = c_mass[0];
    center[1] = c_mass[1];

    // the last entry is inv_area, indicated by "// inverse area for mass compensation"
    // where "bool compensate=false"-> if true a negative mass will be included so that  the region wil have zero mass -> probably to add shear?
    // in my case I believe it should be 0
  ...
  // output some maps
  gridmap.writeFits<float>(LensingVariable::KAPPA,"!results_short/particles_kappa.fits");
  //gridmap.writeFits<float>(LensingVariable::INVMAG,"!particles_invmag.fits");
  gridmap.writeFits<float>(LensingVariable::ALPHA1,"!results_short/particles_ALPHA1.fits");
  gridmap.writeFits<float>(LensingVariable::ALPHA2,"!results_short/particles_ALPHA2.fits");
