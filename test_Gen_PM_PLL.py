# copy from Gen_PM_PLL -> to test 
# eg fix z_source to 4 and see if we get something decent
import os,pickle
...
from python_tools.tools import mkdir,short_SciNot
...
def get_LensSystem_kwrgs(deltaPix,pixel_num=pixel_num,background_rms=0.005,exp_time=500):
    ...
    pixel_num = pixel_num       # pix -> this DOES NOT have to be exactly the same as pixel_num, but it is a reasonable number
...
from Gen_PM_PLL import Lens_PART,plot_all
"""
class Lens_PART(): 
    def __init__(self,
                    Galaxy,
                    kwlens_part, # if PM or AS, and if so size of the core
                    radius=3, # probably could compute it in a smart way... -> prob. read it from Galaxy-> Still to well define -> def in arcsec->def 3''
                    ...
                    savedir_sim="lensing",reload=False # saving params
                    ):
        Galaxy           = prep_Gal(Galaxy) # just set up some directories
        ...
        print("TEST")
        self.z_source    = 4 #self.kwres_dens["z_source"]
        self.proj_index  = 0 # self.kwres_dens["proj_index"]
        print("z_source = ",self.z_source)
        print("proj_index = ",self.proj_index)
    ...
    def setup_lenses(self):
        ...
        kw_samples = Gal2kw_samples(self.Gal,self.proj_index,self.arcXkpc)
...
def Gal2kw_samples(Gal,proj_index,arcXkpc):
    # Particle masses
    ...
    kw_samples["cm"]   = RA_cm-RA_cm,DEC_cm-DEC_cm  # 
...
def get_kappa(Model,plot=True,savename="comp_kappa.png",skip_show=False):
    kw_samples = Gal2kw_samples(Model.Gal,Model.proj_index,Model.arcXkpc)
    samples    = kw_samples["RAs"],kw_samples["DECs"]

    return _get_kappa(imageModel= Model.imageModel_PART,
                                 lens_model=Model.lens_model_PART,
                                 kwargs_lens = Model.kwargs_lens_PART,
                                 samples = samples,
                                 weights = Model.Mscale_sun*np.ones(Model.n_smpl),
                                 arcXkpc = Model.arcXkpc,
                                 SigCrit = Model.SigCrit,
                                 plot=plot,savename=savename,skip_show=skip_show)
...
def _get_kappa(imageModel,samples,Ms,
    ...
    RA,DEC   = util.array2image(_ra),util.array2image(_dec)
    ra0,dec0 = RA[0],DEC.T[0] # center of the bin
    ...
    Dra01   = np.diff(ra_edges)*u.arcsec # ugly, could be cleaner-> but after all it's constant so 
    Ddec01  = np.diff(dec_edges)*u.arcsec
    ...
    if plot:
        plot_kappamap(kappa_PART,extent_kpc,title1=r"$\kappa$ Part.",savename=savename,skip_show=skip_show)
    return kappa_PART,extent_kpc
...
def plot_lensed_im(Model,savename="lensed_im.pdf",skip_show=False):
    image_sim_PART  = Model.image_sim_PART 
    
    fg,ax = plt.subplots(1,figsize=(5,5))
    im0   = ax.matshow(np.log10(image_sim_PART),origin='lower')
    ax.set_title(r"log$_{10}$ ")
    divider = make_axes_locatable(ax)
    cax = divider.append_axes('right', size='5%', pad=0.05)
...
if __name__ == "__main__":
    ...
    mod_LP = Lens_PART(Galaxy=Gal,kwlens_part=kwlens_part_AS,radius=2.3,pixel_num=200,reload=False)
    print("TEST")
    mod_LP.z_source    = 4
    mod_LP.proj_index  = 0 # self.kwres_dens["proj_index"]
    print("z_source = ",mod_LP.z_source)
    print("proj_index = ",mod_LP.proj_index)
    
    mod_LP.SigCrit     = SigCrit(cosmo=mod_LP.cosmo,z_lens=mod_LP.z_lens,z_source=mod_LP.z_source) # Msun/kpc^2
    
    mod_LP.PMLens      = PMLens(kwlens_part_AS)
    mod_LP.PMLens.setup(mod_LP)
